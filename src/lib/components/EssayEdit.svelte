<script lang="ts">
	...
	import { contentType, type Category, type EssayUpload, type UploadResp } from '$defs/content';
	import { essayUrl, listCatalogue, listCategory } from '$lib/api/content';
	...
	/** 原始数据 */
	export let raw: EssayUpload = {} as any;
	/**本地保存(localStorage)的key*/
	export let saveKey: string | undefined = undefined;
	/** 正文预览 */
	export let preview = false;
	/**上传数据函数*/
	export let uploadEssay: (data: EssayUpload) => MaybePromise<UploadResp<number>>;

	$: if (browser && saveKey) localStorage.setItem(saveKey, JSON.stringify(raw));

	/**现在已经存在的图片数据*/
	const existImgs = raw.imgs;
	...
	const upload = async () => {
		...
		try {
			...
			data.imgs = getImgs();
			//上传内容
			const resp = await uploadEssay(data);
			if (!resp.success) {
				showErr = resp.err;
				errDialogOpen = true;
				return;
			...
			if (saveKey && !dev) localStorage.removeItem(saveKey);
			//跳转
...
<div>
	...
	<label for="content">
		...
		<div class="tab-nav row">
			...
			<div class="col-6 col-lg-3 col-xl-1" class:active={preview} on:click={() => (preview = true)}>
				预览
			</div>
		</div>
		{#if preview}
	...
	<ImgUploader
		existFiles={existImgs}
		bind:transText
		bind:upload={uploadImgFunc}
		bind:getImgs
		bind:txt={raw.content}
	/>
